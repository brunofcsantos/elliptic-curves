...
fn main() {
    let pem_path = "/root/Documents/quamtumuc/pqc-bench/submodules/elliptic-curves/signing_example/test/test_data/p521-private-key.pem";
    let pem_str = fs::read_to_string(pem_path).expect("");

    let secret_key = SecretKey::from_pkcs8_pem(&pem_str).expect("");
